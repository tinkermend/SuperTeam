...
use crate::providers::{ProviderAdapter, ProviderRequest, claude::ClaudeProvider, opencode::OpenCodeProvider};

use super::workspace::{create_task_workspace, cleanup_workspace};
...
pub async fn execute_task(
    ...
    while let Some(event_result) = event_stream.next().await {
        if cancel_token.is_cancelled() {
            let _ = control_plane.fail_task(task.id, "Task cancelled".to_string()).await;
        ...
        match event_result {
            Ok(event) => {
                if let Err(e) = push_event_with_retry(&control_plane, task.id, event).await {
                    let _ = control_plane.fail_task(task.id, format!("Failed to push events: {}", e)).await;
                    cleanup_workspace(&workspace, &config)?;
                    return Err(e);
                }
            }
            Err(e) => {
                let _ = control_plane.fail_task(task.id, format!("Provider execution failed: {}", e)).await;
                cleanup_workspace(&workspace, &config)?;
                return Err(e);
            }
        }
    ...
    control_plane.complete_task(task.id, serde_json::json!({"status": "success"})).await?;
    cleanup_workspace(&workspace, &config)?;
...
fn select_provider(provider_type: &str, config: &RuntimeConfig) -> Result<Box<dyn ProviderAdapter>> {
    match provider_type {
        "claude-code" => {
            ...
            Ok(Box::new(ClaudeProvider::new(&config.providers.claude_code.binary_path)))
        }
        "opencode" => {
            if !config.providers.opencode.enabled {
                anyhow::bail!("OpenCode provider is disabled");
            }
            Ok(Box::new(OpenCodeProvider::new(&config.providers.opencode.binary_path)))
        }
