...
type OperationLogRecord struct {
	...
	// ResourceName 资源显示名（读侧从 details 或配置键解析；给人看，不替代 resource_id）
...
type ListOperationLogsParams struct {
	...
	ExcludeModule *string                        `form:"exclude_module,omitempty" json:"exclude_module,omitempty"`
	Since         *time.Time                     `form:"since,omitempty" json:"since,omitempty"`
}
...
func (siw *ServerInterfaceWrapper) ListOperationLogs(w http.ResponseWriter, r *http.Request) {
	...
	// ------------- Optional query parameter "exclude_module" -------------

	err = runtime.BindQueryParameterWithOptions("form", true, false, "exclude_module", r.URL.Query(), &params.ExcludeModule, runtime.BindQueryParameterOptions{Type: "string", Format: ""})
	if err != nil {
		var requiredError *runtime.RequiredParameterError
		if errors.As(err, &requiredError) {
			siw.ErrorHandlerFunc(w, r, &RequiredParamError{ParamName: "exclude_module"})
		} else {
			siw.ErrorHandlerFunc(w, r, &InvalidParamFormatError{ParamName: "exclude_module", Err: err})
		}
		return
	}
