...
func TestProjectRoutesUseConsoleAuthAndProjectService(t *testing.T) {
	...
	addNodeReq := httptest.NewRequest(http.MethodPut, "/api/v1/projects/"+service.projectID.String()+"/runtime-nodes/"+runtimeNodeID.String(), strings.NewReader(`{"reason":"manual"}`))
	addNodeReq.Header.Set("Content-Type", "application/json")
	addNodeReq.AddCookie(cookie)
	addNodeResp := httptest.NewRecorder()
	server.ServeHTTP(addNodeResp, addNodeReq)
	if addNodeResp.Code != http.StatusOK {
		t.Fatalf("expected runtime node add to succeed, got %d: %s", addNodeResp.Code, addNodeResp.Body.String())
	}
	if service.addRuntimeNodeReq.TenantID != expectedTenantID || service.addRuntimeNodeReq.ProjectID != service.projectID || service.addRuntimeNodeReq.ActorUserID != user.ID || service.addRuntimeNodeReq.RuntimeNodeID != runtimeNodeID {
		t.Fatalf("expected runtime node add request from auth/path/body, got %#v", service.addRuntimeNodeReq)
	}

	removeNodeReq := httptest.NewRequest(http.MethodDelete, "/api/v1/projects/"+service.projectID.String()+"/runtime-nodes/"+runtimeNodeID.String(), strings.NewReader(`{"reason":"done"}`))
	removeNodeReq.Header.Set("Content-Type", "application/json")
	removeNodeReq.AddCookie(cookie)
	removeNodeResp := httptest.NewRecorder()
	server.ServeHTTP(removeNodeResp, removeNodeReq)
	if removeNodeResp.Code != http.StatusNoContent {
		t.Fatalf("expected runtime node remove to succeed, got %d: %s", removeNodeResp.Code, removeNodeResp.Body.String())
	}
	if service.removeRuntimeNodeReq.TenantID != expectedTenantID || service.removeRuntimeNodeReq.ProjectID != service.projectID || service.removeRuntimeNodeReq.ActorUserID != user.ID || service.removeRuntimeNodeReq.Reason != "done" {
		t.Fatalf("expected runtime node remove request from auth/path/body, got %#v", service.removeRuntimeNodeReq)
	}
...
type routeProjectService struct {
	...
	overviewCalls                     int
	addRuntimeNodeReq                 project.ModifyProjectRuntimeNodeRequest
	removeRuntimeNodeReq              project.ModifyProjectRuntimeNodeRequest
...
func (s *routeProjectService) AddProjectRuntimeNode(ctx context.Context, req project.ModifyProjectRuntimeNodeRequest) (*project.ProjectRuntimeNode, error) {
