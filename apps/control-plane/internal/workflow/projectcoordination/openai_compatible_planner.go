...
func buildPlannerSystemPrompt() string {
	return strings.Join([]string{
		...
		"Each plan_acceptance_criteria entry must also declare verification_method: automated_test when the criterion can be decided from command or test evidence (satisfied_by is then required), or human_judgment when it is a business or intent judgment only a human can make (satisfied_by may be empty). Also declare severity: blocking (default; failing it blocks acceptance) or non_blocking. Prefer automated_test whenever evidence can decide the criterion. Emit at most ONE blocking human_judgment criterion (the single business/intent gate); any additional human checklist items must be non_blocking. statement must be a decidable assertion written in Simplified Chinese (中文), not English; never emit vague qualifiers like \"尽量\", \"适当\", or \"优化一下\".",
