...
export function ProjectPolicyStep({ draft, onChange }: ProjectPolicyStepProps) {
  return (
    <div className="grid gap-6">
      <div>
        <h3 className="text-xl font-semibold text-ink">协调策略</h3>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        {presets.map((preset) => {
          ...
          return (
            <button
              className={cn(
                "rounded-inner border bg-card p-4 text-left transition hover:border-brand/50",
            ...
            >
              <div className="flex items-center gap-2 text-sm font-semibold text-ink">
                <span
                  className={cn(
                    ...
                    active ? "bg-brand-soft text-brand" : "bg-mute-soft text-mute",
                  )}
                >
                  {preset.icon}
                </span>
              ...
              <p className="mt-3 text-xs leading-5 text-ink-2">{preset.description}</p>
            </button>
          );
        })}
      </div>

      <div className="divide-y divide-line rounded-inner border border-line bg-card">
        <PolicyToggle
