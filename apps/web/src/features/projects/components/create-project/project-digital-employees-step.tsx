...
import { StatusPill, WorkSurface , Button} from "@/components/superteam";
...
export function ProjectDigitalEmployeesStep({
  ...
  return (
    <div className="grid gap-5">
      <div>
        <h3 className="text-xl font-semibold text-ink">选择数字员工池</h3>
        <p className="mt-1 text-sm text-ink-2">仅从项目数字员工池中选取执行员工；人类负责人不归入数字员工。</p>
      </div>

      <section className="grid gap-3 rounded-xl border border-line bg-card-soft p-4">
        <div>
          <h4 className="text-sm font-semibold text-ink">数字员工来源团队</h4>
          <p className="mt-1 text-xs text-ink-3">来源团队只用于筛选候选数字员工，不会作为项目团队成员提交。</p>
        </div>
        {selectableTeams.length === 0 ? (
          <p className="text-sm text-ink-2">暂无可选团队</p>
        ) : (
          <div className="flex flex-wrap gap-2">
            {selectableTeams.map((scope) => {
              ...
              return (
                <Button
                  aria-pressed={active}
                  className={cn("rounded-xl", active && "border-brand bg-brand-soft text-brand")}
                  key={scope.id}
      ...
      <WorkSurface>
        <div className="grid grid-cols-[3rem_minmax(12rem,1fr)_1fr_7rem] border-b border-line bg-card-soft px-4 py-3 text-xs font-semibold text-ink-2">
          <span />
          <span>数字员工</span>
          <span>能力标签</span>
          <span>状态</span>
        ...
        {isEmployeesLoading ? (
          <p className="px-4 py-6 text-sm text-ink-2">正在加载数字员工...</p>
        ) : isEmployeesError ? (
          <p className="px-4 py-6 text-sm text-danger">数字员工加载失败</p>
        ) : employees.length === 0 ? (
          <p className="px-4 py-6 text-sm text-ink-2">{draft.sourceTeamIds.length > 0 ? "暂无匹配数字员工" : "请先选择数字员工来源团队"}</p>
        ) : (
          employees.map((employee) => {
            ...
            return (
              <div
                ...
                className="grid w-full cursor-pointer grid-cols-[3rem_minmax(12rem,1fr)_1fr_7rem] items-center border-b border-line px-4 py-3 text-left last:border-b-0 hover:bg-card-soft focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-brand/10"
                key={employee.id}
              ...
              >
                <span
                  ...
                  className={cn(
                    ...
                    selected ? "border-brand bg-brand-soft text-ok" : "border-line-strong bg-card text-ink-3",
                  )}
                >
                ...
                <div className="flex min-w-0 flex-wrap gap-1.5">
                  {employeeCapabilities(employee).map((capability) => (
                    <span className="rounded-lg bg-mute-soft px-2 py-1 text-xs font-medium text-mute" key={capability}>
                      {capability}
                    </span>
                  ))}
                </div>
...
function FilterButton({
  ...
  return (
    <Button className={cn("rounded-xl", active && "border-brand bg-brand-soft text-brand")} onClick={onClick} type="button" variant="outline">
      {children}
    </Button>
  );
}
