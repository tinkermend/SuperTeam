  ...
  return (
    <SoftCard className="overflow-hidden">
        ...
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              <h3 className="font-semibold text-ink">配置修订历史</h3>
      ...
      {isLoading && revisions.length === 0 ? (
        <LoadingState label="正在加载配置修订历史" />
      ) : null}

      {!isLoading && revisions.length === 0 ? (
        <EmptyState title="暂无配置修订历史" />
      ) : null}
        ...
        <div className="grid gap-0 lg:grid-cols-[280px_minmax(0,1fr)]">
          ...
          <div className="min-w-0 p-4">
            {selectedRevision ? (
              ...
              <div className="text-sm text-ink-2">请选择一个 revision</div>
            )}
          </div>
        </div>
      ) : null}
  ...
  return (
    <div className="grid gap-4">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        ...
        <dl className="grid gap-1 text-xs text-ink-2 sm:grid-cols-2 lg:min-w-80">
      ...
      <div className="grid gap-2">
        <div className="flex items-center justify-between gap-3">
          <h5 className="text-sm font-semibold text-ink">策略对比</h5>
        ...
        <div className="grid gap-3 xl:grid-cols-3">
          {policySections.map((section) => {
            ...
            return (
              <section
                className="min-w-0 rounded-inner border border-line bg-card-soft p-3"
                key={section.title}
              >
                <div className="mb-2 flex items-center justify-between gap-2">
                  <div className="flex min-w-0 items-center gap-2">
                    <Icon className="size-4 shrink-0 text-brand" />
                    <h6 className="truncate text-sm font-medium text-ink">
                      {section.title}
                    </h6>
                  </div>
                ...
                <pre className="max-h-64 overflow-auto rounded-md border border-line bg-card p-3 font-mono text-xs leading-5 text-ink">
                  {stringifyJson(value)}
                </pre>
              </section>
            );
...
function RevisionMeta({ label, value }: { label: string; value: ReactNode }) {
  return (
    <div className="min-w-0 rounded-md border border-line bg-card-soft px-2.5 py-2">
      <dt>{label}</dt>
