    ...
    <div className="flex min-w-0 flex-col gap-4">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <h2 className="truncate text-lg font-semibold text-v3-ink">{employee.name}</h2>
          <p className="text-sm text-v3-ink-2">{employee.roleLabel}</p>
        </div>
...
function StatusBlock({ employee }: { employee: RuntimeOverviewEmployee }) {
  ...
  return (
    <div className="min-w-0">
      <div className="text-xs font-semibold text-v3-ink-2">运行状态</div>
      <div className="v3-glass-inner mt-2 px-3 py-3 text-sm" data-employee-status-block>
        <div className="flex flex-wrap items-center gap-2">
          <span className={`size-2.5 rounded-full ${statusDotClass[employee.status]}`} aria-hidden />
          <span className="font-semibold text-v3-ink">{statusLabel[employee.status]}</span>
          {employee.statusSince ? (
            <span className="text-v3-ink-3 tabular-nums">
              · {statusDurationVerb[employee.status]} {formatDurationSince(employee.statusSince)}
            </span>
          ) : null}
        </div>
        {employee.statusReasons.map((reason) => (
          <p key={reason} className="mt-1.5 text-v3-ink-2">
            {reason}
          </p>
        ))}
        {showError ? <p className="mt-1.5 line-clamp-2 break-all text-v3-danger">{employee.latestRunErrorMessage}</p> : null}
      </div>
    </div>
  );
}
    ...
    <div className="min-w-0">
      ...
      <div className="v3-glass-inner mt-2 px-3 py-3">
        <div className="flex items-start justify-between gap-3">
          <p className="min-w-0 text-sm font-semibold text-v3-ink">{employee.currentTask?.title ?? "暂无进行中的任务"}</p>
          {employee.currentTask?.priority === "high" ? (
            <span className="shrink-0 rounded-lg bg-v3-danger-soft px-2 py-1 text-xs font-semibold text-v3-danger">优先级：高</span>
          ) : null}
        </div>
        {currentProject ? (
          <p className="mt-2 flex items-center gap-1.5 text-sm" data-employee-current-project>
            <span className="text-v3-ink-3">所属项目</span>
            <Link
              params={{ projectId: currentProject.projectId }}
              to="/projects/$projectId"
              className="min-w-0 truncate font-medium text-v3-brand hover:underline"
            >
              {currentProject.name}
            </Link>
          </p>
