...
- [ ] 2026-07-20 多人类负责人模型(用户拍板:多个平级 owner,都能审批/验收,≥1)——**设计 spec 已出待审**:`docs/superpowers/specs/2026-07-20-project-multiple-human-owners.md`。照团队先例(migration 011 单→数组)+项目独有难点是协调线程决策路由扇出;blast radius:数据 scalar→`human_owner_user_ids[]`、authz 数组化、coordinator `TargetUserID` 单→扇出per-owner任一可决、≥1 护栏、Feishu ∪全owners、概览 owners 列表、CLAUDE.md 协作模型改"至少一个平级负责人"。审后按 spec §9 分期(P1后端/P2前端/P3宪法),勿 ad-hoc 改
