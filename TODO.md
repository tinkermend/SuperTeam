...
- [ ] 2026-07-26 `verify:web` 两个 flaky 用例（门禁可信度）：`components/layout/sidebar-menu-sizing.test.tsx > uses readable 15px labels...` 与 `features/inbox/index.test.tsx > guards rapid duplicate action submissions`，全量跑偶发失败、单跑必过（后者是测试自身与被测的防重复提交抢时序）。当前只能靠复跑规避，会掩盖真回归
