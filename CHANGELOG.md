...
- 2026-07-05 04:24：流程编排页在 task graph 尚无任务但存在协调阻塞事实时，显示“协调已阻塞”横幅和独立 `workflowBlocking` 画布节点，不再把 Runtime placement 等阻塞事实伪装成任务节点；空任务图仍保留真实 graph 画布用于展示阻塞原因和下一步动作。验证：`corepack pnpm --filter ./apps/web run test -- src/features/workflows/workflow-graph-adapter.test.ts src/features/workflows/index.test.tsx src/features/workflows/components/workflow-task-node.test.tsx`、`git diff --check` 通过。
