...
- 2026-06-22 19:54：ProjectTask Attempt 结构化结果补齐非完成态恢复路径：`SubmitProjectTaskAttemptResult` 在记录并链接最新 `TaskResultContract` 后，对 `revision_needed`/contract-changed revision 与 `blocked` 进入既有 waiting-human 决策路径，对显式 `replan_request` 进入 `plan_invalid` 人工等待，对 retryable `failed` 复用 attempt failure retry 语义，对 final failed/cancelled 复用失败/取消 writeback，避免非完成结果释放下游。验证：新增 RED/GREEN service tests，`go test ./apps/control-plane/internal/project -run 'Test(SubmitProjectTaskAttemptResult|CompleteProjectTaskAttempt|ValidateTaskResultContract|TaskResultContract|PgRepositoryListUnresolvedBlockersRequiresAcceptedLatestResult)' -count=1`、`go test ./apps/control-plane/internal/project -count=1`、`go test ./apps/control-plane/internal/workflow/projectcoordination -count=1`、`git diff --check` 通过；本任务按要求未做 live smoke。
