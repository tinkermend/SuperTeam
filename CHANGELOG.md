...
- 2026-07-17 20:34（分支 feat/directory-capability-projection-p3，待合并窗口 E2E）：目录与能力投影修订 Phase 3——清理闭环（spec `2026-07-17-directory-capability-projection-revision-design.md` §5，还清"cleanup_policy/max_retained 死配置"断点 3）。新模块 `workspace_cleanup.rs`：① **终态清理**——run 事件流收尾在 artifacts/attestation 采集与终态回写全部落地后按 `workspace.cleanup_policy` 处理该 attempt 任务工作区（on_success 缺省成功清失败留排障/always/never），删除后 `git worktree prune` 清悬空注册；安全闸=仅 `{base}/workspaces/{proj}/{task}/{attempt}` 恰好三段形态可清，chat 线程目录与未知路径在 plan 层即拒绝；drain 早退 `?` 路径跳过钩子由后台清扫兜底。② **后台清扫（janitor）**——命令循环启动时挂起,60s 首扫此后每 30 分钟：任务工作区按项目 LRU 裁剪至 `max_retained`（空壳 task 目录顺手清）,chat 线程目录 TTL（新配置 `chat_ttl_days` 缺省 7 天,活动时间=目录+一级子项 mtime 最大值）+ 每项目条数兜底（`chat_max_retained` 缺省 20;spec 原文"每(项目,员工)5"因路径不编码员工调整为每项目口径,spec §5 同步回写）;活跃 run 引用目录一律跳过（`RuntimeRunStore::active_workspaces` 按 Running 状态取）;清扫在 spawn_blocking 中执行不阻塞事件循环。配置新增 env `RUNTIME_AGENT_CHAT_TTL_DAYS`/`RUNTIME_AGENT_MAX_RETAINED_CHAT_THREADS`。验证：新单测 6 例（策略三态/chat 与异形路径拒绝/LRU+活跃跳过/TTL+条数兜底/一级深度活动时间）+ 全量 109 lib 测试 + `verify:runtime-agent` 绿。真实 E2E 留合并窗口：成功任务工作区自动消亡/失败保留、chat 线程存活且旧残迹被 TTL 清扫、`git worktree list` 无悬空。
