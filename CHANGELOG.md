...
- 2026-06-28 23:56：修复任务发起「浏览模板库」弹窗模板卡片网格列数错配：`PromptTemplateDialog` 模板网格原用视口断点 `sm:grid-cols-2 xl:grid-cols-3`，在 `max-w-[900px]` 弹窗内容区（约 660px）内会被 ≥1280px 视口的 `xl` 断点压成 3 列，每张卡片仅约 210px 过窄、`line-clamp-3` 正文过早截断；改为 `grid-cols-[repeat(auto-fill,minmax(15rem,1fr))]`，列数跟随容器宽度，在该弹窗内稳定 2 列、卡片宽约 320px，容器更宽时自动 3 列。验证：用真实 chromium 渲染 v3 复刻页对比，卡片宽度 209→321px、正文由截断变为完整；`corepack pnpm --filter ./apps/web run typecheck` 零新增错误（stash 对比确认 3 个既有错误与本改动无关）；`git diff --check` 通过。局部验证：未在真实运行 Web 的任务发起页打开该弹窗做浏览器实操（playwright MCP 浏览器扩展未安装、dev 登录未走通），建议用 codex chrome 在 `/task-launches` 点击「✨ 浏览模板库」确认。
