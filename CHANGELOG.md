...
- 2026-06-12 23:36：收件箱团队只读视图接入现有授权器，具备租户级 `team.read` 权限时开放团队列表和 badge 团队数量，未授权仍按个人范围返回。
- 2026-06-12 23:31：收紧收件箱团队视图默认授权，未接入团队视图 authorizer 前拒绝团队列表并隐藏 badge 中的租户级团队待办数量，同时补充团队待办查询索引。
