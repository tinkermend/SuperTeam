...
- 2026-06-23 00:27：ProjectTask 结果人审验收写回补齐 Phase 4 gating 事实：`project_task_acceptance` 人工批准时，如果等待中的任务最新结果为 `waiting_human_review`，追加新的 append-only `project_task_results` 行并设为 latest（`result_status=completed`、`validation_status=accepted`、`decision=complete_accepted`），保留原始结果历史，在 contract 的 `human_accepted_reason` 记录人类验收意见，并将新结果链接到对应 `decision_request`；批准完成任务后使用人工处理事件发送 `EmployeeTaskCompleted` 协调信号以释放下游依赖。非 approve 分支不写 `complete_accepted` 结果、不发送 completed signal。验证：`go test ./apps/control-plane/internal/project -count=1`、`go test ./apps/control-plane/internal/workflow/projectcoordination -count=1`、`git diff --check`、`git diff --cached --check` 通过；本切片未改 contracts/generated files，未跑 `corepack pnpm verify:contracts`。
