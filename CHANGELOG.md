...
- 为 Control Plane 配置 sqlc 生成类型安全的数据访问代码，覆盖 runtime、tasks、auth 和 audit 模块。
- 新增 Runtime 节点 CRUD 查询：注册、心跳更新、负载更新、在线节点列表。
- 新增任务生命周期查询：创建、获取、列表、状态更新、分配、取消、待处理任务查询。
- 新增任务执行、事件、工件和状态历史查询，支持完整的任务追踪。
- 新增用户和 Runtime token 认证查询，支持用户管理和节点认证。
- 新增审计事件查询，支持按 actor、resource、时间范围过滤和统计。
