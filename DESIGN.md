...
极光是当前全站共享的**氛围背景底**，玻璃沉浸是其上按页分层的**受控特例**。两者分开治理：
...
- **Tier A · 叠加玻璃沉浸**（低数据密度、单一主 CTA、录入/创建/入口/落地）：任务发起 `/task-launches`（基准实现）、单流程创建向导（技能上传 `/skills/upload`、整页新建流）、登录与首次进入/onboarding 空状态。
- **Tier B · 只用 Soft Card**（不叠玻璃卡，保留 Soft Card 质感与轻品牌 accent，坐在极光背景上）：首页 hub `/`、运行总览 `/run-overview`、实体目录（数字员工/技能/团队）、各详情头卡。
- **Tier C · 保持不透明脆数据面**（背景仍是极光，但内容必须走不透明高对比脆数据面，**禁止把卡/表改成半透明玻璃**）：任何需逐行扫读、比较、判断的数据本体——项目队列、收件箱、流程编排/运行视图、注册表与配置（自动化/外部能力/MCP/协作集成）、审批队列、Runtime 监控、权限/成本/用户治理表格。**审计中心 `/audit`、日志管理 `/logs` 尤其严格**：内容面不得半透明、不得模糊，保证可读性与可审计可信度。

一旦某屏从"入口/创建"演进为承载密集数据或审计，必须退回不透明脆数据面，不得为保持观感把内容面继续套玻璃。
