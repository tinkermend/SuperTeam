...
项目不是只表示软件交付项目，也可以表示一次具体问题场景的闭环。项目是目标、负责人、协调者、任务、证据、预算、审批和验收结论的聚合容器；流程编排是驱动项目运行的模板，不替代项目作为业务事实入口。
...
- 一个 Project 绑定一个项目协调员。项目协调员是调度型数字员工，不是人类管理者；只负责分派、跟踪、汇总、证据检查和汇报生成，不承担最终业务责任，不直接审批高风险动作，不直接执行专业任务，不默认读取完整原始上下文。
- 每个项目协调员必须绑定固定人类归属人，并可绑定 leader 或验收人。人类归属人负责最终业务判断、审批、驳回、补证要求、汇报接收和验收结论。
- 人类员工和数字员工都是项目成员，但职责不同。人类可以作为归属人、审批人、验收人、专家执行者或观察者；数字员工可以作为项目协调员或执行员工。不要把人类管理职责建模成数字员工能力，也不要让数字员工绕过人类决策。
- Agent 之间不直接自由聊天，通过结构化对象协作；每个阶段都必须产出可持久化的工件、证据、决策或交接包。项目协调员控制平面的详细对象和流程规则见 `docs/design/global/project-coordinator-control-plane.md`。
