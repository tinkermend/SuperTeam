...
## Soft-Flat 表面模型：一套语言，三种容器（当前目标）

Soft-Flat 用**三种容器**承载所有内容，按"页面 Tier 分类 + 是否需要逐行扫读"选择，三者共用同一套 token：
...
- 形态：`background: var(--card)`（纯白/暗色实底），`border-radius: var(--r-card ~22px)`，弥散浅阴影 `var(--shadow)`，无硬边框或仅极淡 `--line`。
- 内容：黑色粗体大数字 + 灰色小标签；自带“状态 pill + 关键指标 + 一个主操作”；hover 可轻微上浮。
- signature：每个概览屏最多一块 signature 卡做视觉锚点与重心，承载该页最有故事性的信息（环形进度、步骤、聚合分布、命令入口）。
  - 表面优先使用实底 `--card` / `--signature-surface` + 品牌顶线 + 低对比网格 + 克制节点/连线。
...
- 形态：**实底、不透明、不模糊、零装饰**。表头 `--card-soft` + sticky；行线 `--line`；hover 行用 `--card-inner`。
- 纪律：数字 `tabular-nums`；ID/UUID/路径用等宽字体；危险/失败行用**左侧实色 accent bar（`box-shadow: inset 3px 0 0 var(--danger)`）+ 实底浅红**，不靠半透明削弱；提供"舒适/紧凑"密度切换（行内边距 token 化）。
...
| 属性 | Token | 浅色值 | 深色值 |
| --- | --- | --- | --- |
| 玻璃卡背景 | `--aurora-glass` | `rgba(255,255,255,0.55)` | `rgba(22,28,44,0.55)` |
| 玻璃卡边框 | `--aurora-glass-border` | `rgba(255,255,255,0.8)` | `rgba(120,150,255,0.16)` |
| 内层面板背景 | `--aurora-panel` | `rgba(255,255,255,0.7)` | `rgba(18,24,40,0.6)` |
| 内层面板边框 | `--aurora-panel-border` | `rgba(47,95,255,0.18)` | `rgba(120,150,255,0.2)` |
| 模糊半径 | `--shell-glass-blur` | `22px` | `22px` |
| 饱和度增强 | `--shell-glass-saturate` | `1.35` | `1.24` |
| 卡片圆角 | `--aurora-r-card` | `24px` | `24px` |
| 内层圆角 | `--aurora-r-inner` | `16px` | `16px` |

**唯一实现（不要重抄、不要新建平行 `*-glass` 类）**：

- 组件：`GlassCard`（`@/components/superteam`）——Tier A 页面统一用 `<GlassCard>` 做玻璃外壳。
- 样式类：`.glass` / `.glass-inner`，定义在 `apps/web/src/styles/index.css` 的 `@layer components`，色值/圆角/模糊全部取自 `--aurora-*` / `--shell-glass-*` token（含 `prefers-reduced-motion` 降级）。
- feature 内的 `*aurora.css`（`.tl-*` 等）只承载**页面专属布局**，不得重新声明玻璃表面。

`.glass` 等价样式（仅供理解，实际以 `index.css` 为准）：

```css
.glass {
  background: var(--aurora-glass);
  border: 1px solid var(--aurora-glass-border);
  border-radius: var(--aurora-r-card);
...
.glass-inner {
  ...
  border-radius: var(--aurora-r-inner);
}
```

**内层结构**：
- 玻璃卡内部可嵌套 `.glass-inner` 做二级面板（表单区、摘要卡、预检面板），保持层级一致。
- 玻璃卡内部的表单输入框、select 使用 `rgba(255,255,255,0.9)` 实底（非半透明），保证可读性。
- 玻璃卡内部的按钮遵循 `Button` 规范：次级按钮用 `variant="glass"`（半透明 aurora 面板 + 品牌 accent，融入玻璃），主操作可用 `linear-gradient(180deg, #4f8cff, #2f5fff)` 品牌渐变；不要在玻璃上放不透明白底按钮。
- 玻璃卡内部的二级面板（当前任务、消耗、摘要等）用 `.glass-inner`；进度条/分隔等细线用 `--aurora-hairline`，不要用纯白实底。
...
- **软壳装脆数据**：密集表格被装进一张柔和白卡（`--r-card` 圆角 + 弥散阴影）里——外缘柔、内部逐行脆。
- **玻璃壳装实底内核**：低密度面可用玻璃卡做外壳，但内部表单输入、密集列表和逐行数据仍须退回实底（`rgba(255,255,255,0.9)` 或 `--card`），不把每行/每格套半透明。
- **内容区必须实底**：轻量半透明手势只允许出现在顶栏控件或吸顶兜底层；顶栏容器可以透明融入 Shell，但一旦进入逐行内容，背景必须实底高对比。
- **默认安静、例外才喧哗**：界面大面积保持中性灰，只有需要人介入的对象（待审批、失败、危险）才着色/加粗跳出。

## 背景

- 控制台外壳使用 Acrylic Shell：`--shell-bg` 做柔和底色，`--shell-background` 承载低对比渐变 wash，`--shell-grid` 只作为极淡网格层。
- 顶部 Header 默认透明，让 Shell 背景贯穿；搜索框、主题、通知和账户等控件使用独立 acrylic surface，其中搜索框用更强的 `--shell-search*` token 承担视觉中心。
- 主内容里的卡片、表格、表单和详情面仍使用 `--card` / `--card-soft` / `--bg` 等实底 token，不直接透出背景纹理。
...
- 极微弱对角渐变，例如从 `--card` 过渡到 `--card-soft` 或 `--brand` 的浅软底。
- 细粒度内侧高光，例如 `ring-1 ring-white/60`
- hover 上浮，例如 `hover:-translate-y-1`
- 使用 `--shadow-hover` 的柔和阴影

高级卡片质感只用于真正独立的卡片对象，不用于每个行、字段或小分组。

## 复用表面组件

- 登录页和控制台外层背景应复用同一套 Soft-Flat Shell 组合。
- 通用卡片应复用 Soft Card 组合。
- 搜索框、Tab 容器、次要胶囊按钮、轻量筛选器和非状态型短标签应复用 Soft-Flat pill 组合。
