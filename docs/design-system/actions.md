...
主按钮使用 `@/components/superteam` 导出的 `Button`（Soft-Flat 唯一业务按钮）。`components/ui/button` 仅为内部 primitive（data-table / Radix 复合组件等白名单），**业务 features/routes 禁止新增引用**。迁移细节见 `docs/design-system/migrations/2026-07-24-soft-flat-naming-unification/`。只用于局部流程里的主要正向动作。

建议视觉：

- 品牌色：`--brand`，需要 signature 强调时可使用小面积 `--brand-grad`。
- 文字和图标对比度必须充足
- 不叠加高光层或局部装饰光效。
- 阴影应柔和且受控，使用 `--shadow` 或更轻的焦点 ring。
- active 态应像按下，而不是跳动
...
- 快捷键使用 `Kbd`；计数和 loading 文案保持宽度稳定，避免输入区或工具栏跳动。
...
- 工具栏上「互斥/并列」次级操作可用 `ButtonGroup` 贴合排列。
- 组内仍只用一个最高权重主操作（通常组外另放 primary）。
