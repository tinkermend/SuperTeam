...
采用“React 控制台外壳 + Konva/Canvas 地图层 + 数据配置工位”的方案。

- React 负责页面框架、筛选、楼层切换、团队卡片、右侧详情、状态图例和普通表单/按钮。
- Konva/Canvas 负责中间办公地图：楼层底图、团队办公区、工位、头像、状态点、选中光圈、路径连线和缩放。
...
选择这个方案的原因：它比 DOM 绝对定位更适合大量坐标和缩放，比整图截图更可维护，也比动态排版办公区更稳定。固定 10 工位让画布、审计和用户记忆都保持稳定。
...
1. 画布背景：浅灰网格、地面纹理或干净底图资产。
2. 路径与楼层结构：墙体、门、虚线路径、楼层动线。
3. 团队区域：多边形边界、办公桌、椅子、屏幕、绿植等装饰。
4. 员工层：头像、状态点、选中光圈、告警标记。
5. 流转层：任务流转虚线、运行中的弱动画线。
6. 交互命中层：员工和团队 hit area。
7. HTML 浮层：团队卡片、工具条、右侧详情。

团队卡片建议用 HTML 浮层而不是 Canvas 文本，这样能复用项目字体、状态 pill、可访问性和响应式文本处理。
...
- `RuntimeMapCanvas`：Konva/Canvas 地图根组件。
- `FloorRenderer`：当前楼层底图、路径、团队区。
