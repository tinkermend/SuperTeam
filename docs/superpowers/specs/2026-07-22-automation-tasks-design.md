...
```
┌─ 页头：标题 · 副标题 · [新建规则] ─────────────────────────┐
├─ MetricGrid（总数 / 启用中 / 近窗失败）─────────────────────┤
├─ 工具栏：搜索名称 · 筛选(全部|启用|已禁用) · 模式 chip 可选 ──┤
├────────────────────────────┬──────────────────────────────┤
│ 规则表 WorkSurface+V3Table │ 详情轨（选中后出现）            │
│ 整行可选中                  │ · 规则摘要 + 人类闸门提示       │
│                            │ · 启停 / 立即试跑 / 编辑 / 删除 │
│                            │ · 最近触发（fires）列表         │
└────────────────────────────┴──────────────────────────────┘
         │
         └─ 新建/编辑 → Sheet 抽屉（非全页向导）
```

布局实现优先复用 `MasterDetailLayout`（detail 仅在选中时传入；禁常驻空右栏）。
...
- 不做日历热力图 / 甘特式调度可视化。
- 不做规则卡片网格（身份目录风）——本页是配置审计表，不是实体橱窗。
