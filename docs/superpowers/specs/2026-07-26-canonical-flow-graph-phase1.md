...
- **Phase 1**：本 spec。
- **Phase 2**：权威图落进项目详情需求维度（合并 TODO 07-22 横向管道）；河道列表迁任务中枢并接通 `q`/`project_id` 过滤；`/workflows/:demandId` 留重定向壳（飞书深链兼容），菜单撤"流程编排"；血缘面板归位验收域；KPI 口径收敛（含"已归档"页签更名、恒零"已完成"卡处置）；执行轨迹面板补按任务过滤/高亮（Phase 1 深链只到面板级，2026-07-26 人类拍板延至本期与项目详情重做合并动）。
