...
CREATE INDEX idx_project_events_tenant_project_sequence
    ON project_events(tenant_id, project_id, sequence_number DESC);
```

服务层写事件时必须在事务内分配项目内递增序号，可以通过锁定项目行、独立计数器行或等效机制实现；如果并发写入触发唯一冲突，必须重试或返回明确错误，不能静默覆盖事件。
...
- 项目事件必须有 `UNIQUE(project_id, sequence_number)` 数据库约束作为并发兜底。
- 所有写操作必须校验租户与团队范围。
...
`overview` 使用独立子路径，而不是 `GET /projects/{projectId}?include=overview`。这是为了和当前 Control Plane 已有的 `/api/v1/runtime/overview`、`/api/v1/teams/{teamId}/overview`、`/api/v1/digital-employees/overview` 风格保持一致；如果后续全局 REST 规范统一改为 include 参数，可以再增加兼容别名，但 V0 不需要为此改动主路径。
