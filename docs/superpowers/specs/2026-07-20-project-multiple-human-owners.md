...
- 日期：2026-07-20
- 状态：P1a 已实施并真实 E2E 验证入 main（6d4e8a74/faebec89/29afd394/ffd5845e）；**P1b 决策扇出经勘察证实冗余，已取消**（见下「§5 修正」）；剩余 Web + P1c。
...
- 已锁定决策：owners 等同于 owner-role 的人类成员（数组为去规范缓存,成员行为事实源）。
...
- ~~待人类确认：决策路由方案 / owners 定义~~ → **已确认 2026-07-20：路由取 (A) 扇出；owners 等同 owner-role 人类成员。**
