...
- 状态：待评审
- 日期：2026-07-17
> 复核状态：已实施，现状与本设计一致：P1（绑定+审批卡+结果通知+long connection）已合并main，后续07-19/07-27批次进一步完善卡内签署/心跳可观测/双人any-of-N真实验证，均在原架构方向内延展未被取代。
...
- 卡片分级演进：§8.2 为 P1 初版；按 HumanTask `kind` 的正式交互分级与负荷原则见 `2026-07-25-human-task-load-budget-and-channel-grading.md` §5（冲突时以该文为准）。
