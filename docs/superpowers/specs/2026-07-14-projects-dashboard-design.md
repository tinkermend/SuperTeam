...
- 日期：2026-07-14
> 复核状态：**部分被 2026-08-10 方案取代**（`2026-08-10-projects-home-portfolio-hygiene-design.md` v2）。列表态骨架（canvas + 队列 + 选中 triage）保留；默认右栏「待我决策 + 最近运行」与 KPI「待我决策」第 5 卡已废止，改为组合透视 + 弱链「我的待办」；列表计数改走 `run-summary`。
- 状态：已批准（方向经用户确认；维度取舍按克制默认：决策流 + 运行动态进首页，Runtime 健康/成本留在运行总览，记为可选 P2）— **2026-08-10 修订见上**
