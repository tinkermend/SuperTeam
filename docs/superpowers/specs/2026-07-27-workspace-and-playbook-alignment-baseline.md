...
| 1 | 一单工作台 → [`2026-07-29-demand-workbench-design.md`](./2026-07-29-demand-workbench-design.md)（**立项未实施**） | 项目详情需求视图升级为处所；中栏时间线为主/图为辅；右轨按剧本 kind；两种密度；统一深链；**新增** demand 只读 workbench API | **容器先行**：接续、变更、治理动作都挂它；CP 只读聚合 + Web，无写路径/无迁移 |
