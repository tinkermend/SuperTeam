...
| 切片 | 内容 | 可独立验收 |
|---|---|---|
| **P0a** | recovery 丢根修复（§5.4） | 独立缺陷，先落；单测 + 真实重试 E2E |
| **P0b** | 迁移 + `continues_demand_id` + 接续 API + 链继承规则 | curl 造链，DB 核对 |
| **P0c** | 派发期血缘传播（§5.1/§5.2，含递归 CTE 与 D1–D7） | 多员工真实 E2E（本 spec 的核心判据） |
| **P0d** | 卷宗 lineage 字段 + Web 入口 + 左轨按链折叠 | 浏览器真链路 |
| **P1** | resume 预检降级 + 留痕（§6.1） | 造过期会话验证不失败且留痕 |
